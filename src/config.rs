...
/// The links waiting in the queue, so closing the app doesn't lose them.
pub const QUEUE_NAME: &str = "mimirdlp.queue";
...
keyed!(Subs {
    ...
    File => "file", "Save as a subtitle file",
...
/// Where to take sign-in cookies from: the browsers `yt-dlp --help` lists
/// for `--cookies-from-browser`, less Safari (macOS only).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Browser {
    None,
    Firefox,
...
pub struct Settings {
    ...
    pub chapters: bool,
    pub subs: Subs,
    /// yt-dlp `--sub-langs` syntax. Empty means all languages but the live
    /// chat.
    pub sub_langs: String,
    /// Also take the subtitles a site generates itself (`--write-auto-subs`).
    /// Many YouTube videos have no others.
    pub auto_subs: bool,
    pub sponsor: Sponsor,
    pub cats: Cats,
    ...
    /// Sign in with a browser's cookies.
    pub cookies_browser: Browser,
    /// That browser's profile, by name or path. Empty means the one used
    /// most recently.
...
impl Default for Settings {
    fn default() -> Self {
        ...
        Settings {
            ...
            cookies_browser: Browser::None,
            cookies_profile: String::new(),
        }
    }
}
...
impl Settings {
    ...
    /// in this text field's path does it for us. A relative folder is next
    /// to the application, like the history file, never relative to
    ...
    pub fn output_dir(&self, paths: &Paths) -> PathBuf {
        ...
        {
            return PathBuf::from(home).join(rest.trim_start_matches(std::path::is_separator));
        }
        paths.app_dir.join(dir)
    }
    ...
    pub fn to_options(&self, paths: &Paths) -> Options {
        ...
        // Only offered with the whole playlist: anywhere else a video has no
        // playlist and would land in `NA/NA - title.ext`.
        o.organize_playlist_folders = self.organize && self.playlist == PlaylistChoice::Whole;
        ...
        o.cookies_from_browser = self.cookies();
        o
    }

    /// `--cookies-from-browser`'s value: `BROWSER` or `BROWSER:PROFILE`.
    pub fn cookies(&self) -> Option<String> {
        if self.cookies_browser == Browser::None {
            return None;
        }
        let browser = self.cookies_browser.key();
        Some(match some_text(&self.cookies_profile) {
    ...
    pub(crate) fn serialize(&self) -> String {
        ...
        [
            ...
            ("subtitle_auto", yes(self.auto_subs).into()),
            ...
            ("cookies_browser", self.cookies_browser.key().into()),
            ("cookies_profile", self.cookies_profile.clone()),
        ]
        .into_iter()
        // One setting per line: a pasted line break would otherwise cut the
        // value short and start a key of its own.
    ...
    pub(crate) fn parse(text: &str) -> Settings {
        ...
        for (key, value) in text.lines().filter_map(|line| line.split_once('=')) {
            ...
            match key.trim() {
                ...
                "subtitle_auto" => s.auto_subs = flag(s.auto_subs),
                ...
                "cookies_browser" => {
                    s.cookies_browser = Browser::from_key(value).unwrap_or(s.cookies_browser)
                }
                "cookies_profile" => s.cookies_profile = value.to_string(),
                _ => {}
            }
        }
        s
...
/// One link waiting in the queue, with the settings it was queued with.
#[derive(Debug, Clone, PartialEq)]
pub struct SavedItem {
    pub url: String,
    pub settings: Settings,
    /// Why its last attempt failed, if it did.
    pub failed: Option<String>,
}

const ITEM_MARK: &str = "[item]";

/// Writes the queue next to the application, one `[item]` block per link
/// in the settings file's own `key=value` format. An empty queue removes
/// the file.
pub fn save_queue(paths: &Paths, items: &[SavedItem]) -> std::io::Result<()> {
    let file = paths.app_dir.join(QUEUE_NAME);
    if items.is_empty() {
    ...
    for item in items {
        ...
        text += &item.settings.serialize();
    }
    let tmp = paths.app_dir.join(format!("{QUEUE_NAME}.tmp"));
    fs::write(&tmp, text)?;
    fs::rename(&tmp, &file)
}

/// The queue as it was last saved. A block without a link is skipped.
pub fn load_queue(paths: &Paths) -> Vec<SavedItem> {
    let text = fs::read_to_string(paths.app_dir.join(QUEUE_NAME)).unwrap_or_default();
    text.split(ITEM_MARK)
        .filter_map(|block| {
            let value = |key: &str| {
                block
                    .lines()
            ...
            let url = value("url").filter(|u| valid_url(u))?;
            Some(SavedItem {
                url,
                settings: Settings::parse(block),
                failed: value("failed"),
            })
        })
        .collect()
...
/// A rate like `500K` or `4.2M`. yt-dlp's own rule (`parse_bytes`): digits,
/// optionally a dot and more digits, then optionally one unit letter in
/// either case. Checked against yt-dlp itself, which refuses `1e3`, `+5`,
/// `.5`, `5.` and `10b` although Rust would parse most of them as numbers.
...
pub fn valid_rate(rate: &str) -> bool {
    ...
    let number = rate
        .strip_suffix(|c: char| "kmgtpezy".contains(c.to_ascii_lowercase()))
        .unwrap_or(rate);
    let (whole, fraction) = number.split_once('.').unwrap_or((number, "0"));
    is_number(whole) && is_number(fraction)
}
...
/// `inf` parses as a number, and yt-dlp takes it: it then sleeps forever.
pub fn valid_sleep(value: &str) -> bool {
    matches!(value.trim().parse::<f64>(), Ok(n) if n.is_finite() && n >= 0.0)
}
...
pub fn valid_wait(value: &str) -> bool {
    ...
    match value.split_once('-') {
        // yt-dlp refuses a range whose end comes before its start.
        Some((min, max)) => {
            is_number(min)
                && is_number(max)
                && matches!((min.trim().parse::<u64>(), max.trim().parse::<u64>()), (Ok(a), Ok(b)) if a <= b)
        }
...
/// What `yt-dlp --help` lists for `--remux-video` and `--recode-video`
/// (the same set for both). Anything else fails at argument parsing.
pub const CONTAINERS: &[&str] = &[
    "avi", "flv", "gif", "mkv", "mov", "mp4", "webm", "aac", "aiff", "alac", "flac", "m4a", "mka",
    "mp3", "ogg", "opus", "vorbis", "wav",
];

/// A container yt-dlp can convert to, in any case (`MP4` is fine).
pub fn valid_container(value: &str) -> bool {
    CONTAINERS.contains(&value.trim().to_ascii_lowercase().as_str())
}
...
/// shortened and regional forms it supports. Whitespace inside means more
/// than one link, which the Download screen splits before asking.
...
",
    ...
    fn the_output_folder_defaults_next_to_the_application_and_expands_a_tilde() {
        ...
        // Not a home folder: an ordinary relative name, next to the app.
        settings.output_dir = "~sometool/x".into();
        assert_eq!(
            settings.output_dir(&paths),
            PathBuf::from("/app").join("~sometool/x")
        );
    }

    /// The toggle is only shown with "The whole playlist", but it used to
    /// apply whatever the playlist choice: every single video then landed
    /// in `NA/NA - title.ext`, ignoring the file name template.
    #[test]
    fn playlist_folders_only_apply_to_whole_playlists() {
        let mut settings = Settings {
            playlist: PlaylistChoice::Whole,
            organize: true,
            ..Settings::default()
        };
        assert!(settings.to_options(&paths()).organize_playlist_folders);
        settings.playlist = PlaylistChoice::Auto;
        assert!(!settings.to_options(&paths()).organize_playlist_folders);
        settings.playlist = PlaylistChoice::Single;
        assert!(!settings.to_options(&paths()).organize_playlist_folders);
    }

    /// Like the history file: a relative folder is next to the application,
    /// not wherever the app happened to be started from.
    #[test]
    fn a_relative_output_folder_is_next_to_the_application() {
        let settings = Settings {
            output_dir: "Videos".into(),
            ..Settings::default()
        };
        assert_eq!(
            settings.output_dir(&paths()),
            PathBuf::from("/app").join("Videos")
        );
    }

    /// Checked against yt-dlp 2026.09.16 itself: it refuses `1e3`, `+5`,
    /// `.5`, `5.` and `1.5MiB` as a rate, accepts a lowercase `500k`, and
    /// takes `inf` as a sleep, which means waiting forever.
    #[test]
    fn typed_values_follow_what_yt_dlp_itself_accepts() {
        for ok in ["500K", "4.2M", "500k", "2G", "10", "1.5m"] {
            assert!(valid_rate(ok), "{ok}");
        }
        for bad in ["1e3", "+5", ".5", "5.", "1.5MiB", "inf", "-5K"] {
            assert!(!valid_rate(bad), "{bad}");
        }
        for bad in ["inf", "NaN", "infinity"] {
            assert!(!valid_sleep(bad), "{bad}");
        }
        assert!(valid_wait("60-3600") && !valid_wait("3600-60"));
        // The list `yt-dlp --help` gives for --remux-video/--recode-video.
        assert!(valid_container("mp4") && valid_container("aiff") && valid_container("gif"));
        assert!(!valid_container("xyz") && !valid_container("mp5"));
    }

    #[test]
    fn a_link_with_spaces_inside_is_not_one_link() {
        assert!(!valid_url("https://a.example/x https://b.example/y"));
        assert!(!valid_url("https://a.example/x\nhttps://b.example/y"));
        assert!(valid_url("  https://a.example/x  "));
    }

    /// One pasted line break used to end the value early and turn the
    /// rest into a key of its own.
    #[test]
    fn a_value_with_a_line_break_cannot_rewrite_other_settings() {
        let dir = tempdir();
        let paths = Paths::new(dir.clone());
        // `theme` is written before `custom_template`, so a smuggled line
        // would win over the real one.
        let settings = Settings {
            custom_template: "%(id)s\ntheme=light.%(ext)s".into(),
            ..Settings::default()
        };
        settings.save(&paths).unwrap();
        let loaded = Settings::load(&paths);
        assert_eq!(loaded.theme, ThemePref::System);
        assert_eq!(loaded.custom_template, "%(id)s theme=light.%(ext)s");
        fs::remove_dir_all(dir).unwrap();
    }

    #[test]
    fn browser_cookies_reach_the_engine_with_an_optional_profile() {
        let mut settings = Settings::default();
        assert_eq!(settings.to_options(&paths()).cookies_from_browser, None);
        settings.cookies_browser = Browser::Firefox;
        assert_eq!(settings.cookies().as_deref(), Some("firefox"));
        settings.cookies_profile = " work ".into();
        assert_eq!(
            settings
                .to_options(&paths())
        ...
        let dir = tempdir();
        let p = Paths::new(dir.clone());
        settings.save(&p).unwrap();
        assert_eq!(Settings::load(&p).cookies(), settings.cookies());
        fs::remove_dir_all(dir).unwrap();
    }

    #[test]
    fn the_queue_survives_a_save_and_load_with_each_items_settings() {
        let dir = tempdir();
        let paths = Paths::new(dir.clone());
        let items = vec![
            SavedItem {
            ...
            SavedItem {
                ...
                failed: Some("Video unavailable\nsecond line".into()),
            },
        ];
        save_queue(&paths, &items).unwrap();
        let loaded = load_queue(&paths);
        assert_eq!(loaded.len(), 2);
        assert_eq!(loaded[0], items[0]);
        assert_eq!(loaded[1].settings, items[1].settings);
        assert_eq!(
            loaded[1].failed.as_deref(),
            Some("Video unavailable second line")
        );
        // Nothing left to keep: no file left behind either.
        save_queue(&paths, &[]).unwrap();
        assert!(load_queue(&paths).is_empty());
        assert!(!dir.join(QUEUE_NAME).exists());
        fs::remove_dir_all(dir).unwrap();
    }

    #[test]
    fn automatic_subtitles_reach_the_engine() {
        let settings = Settings {
            subs: Subs::Embed,
            auto_subs: true,
            ..Settings::default()
        ...
        // The label promises nothing the flags don't do.
        assert!(!Subs::File.to_string().contains("srt"));
    }
