...
//! `tests/engine_scenarios.rs`. Only the presentation flags added here are
//! extra, and they are prepended so the engine's argv stays comparable with
//! those scenarios. They were checked against real downloads: progress lines arrive
...
impl Cancel {
    ...
    fn hold(&self, mut child: Child) {
        if let Ok(mut slot) = self.child.lock() {
            // A Stop that came while yt-dlp was starting found nothing to
            // kill. `cancel` sets the flag before it takes the lock, so one
...
pub fn run(
    ...
    if cancel.is_cancelled() {
        emit(Event::Finished(Finish::Cancelled));
        return;
    }
    let tool = paths.tool("yt-dlp");
    let spawned = process::command(&tool)
        .args(&args)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
    ...
    .map(|stream| {
        ...
        thread::spawn(move || {
            ...
            // UTF-8, and a pipe nobody reads any more kills yt-dlp with
            // SIGPIPE (or EPIPE on Windows) on its next write.
    ...
    loop {
        let line = match rx.recv_timeout(POLL) {
            Ok(line) => line,
            // A cancelled run stops reading instead of waiting for the last
            // writer to let go of the pipes, which is what makes Stop feel
            // immediate even if something outlives the kill.
...
mod tests {
    ...
    mod process {
        ...
        fn collect(paths: &Paths, cancel: &Cancel) -> Vec<Event> {
            collect_cancelling_on(paths, cancel, None)
        }

        /// Presses Stop as soon as yt-dlp prints `trigger`, so a test
        /// cancels a run that is really going rather than racing a timer
        /// against the exec guard.
        fn collect_cancelling_on(
            paths: &Paths,
            cancel: &Cancel,
            trigger: Option<&str>,
        ) -> Vec<Event> {
            ...
            run(
                ...
                cancel,
                &mut |e| {
                    if let (Some(trigger), Event::Log(_, line)) = (trigger, &e)
                        && line == trigger
                    {
                        cancel.cancel();
                    }
        ...
        fn cancelling_stops_the_child_and_reports_it_as_cancelled() {
            ...
            let events = collect_cancelling_on(&paths, &cancel, Some("started"));
            assert!(started.elapsed() < std::time::Duration::from_secs(10));
            assert_eq!(events.last(), Some(&Event::Finished(Finish::Cancelled)));
            fs::remove_dir_all(dir).unwrap();
        }

        /// A byte that isn't UTF-8 (a Latin-1 "é" in a path, say) used to
        /// end the reader thread, which closed the pipe and killed yt-dlp
        /// with SIGPIPE on its next line.
        #[test]
        fn output_that_is_not_utf8_does_not_end_the_download() {
            let (dir, paths) = fake_ytdlp(
                "printf '[download] Destination: /home/Jos\\351/v.mkv\\n'\n\
                 i=0; while [ $i -lt 3000 ]; do echo '[ytp] downloading|10|100|NA|NA|NA'; i=$((i+1)); done\n\
                 echo '[ytp-file] /tmp/out.mkv'",
                0,
            );
            let events = collect(&paths, &Cancel::default());
            assert!(
                events.contains(&Event::Log(
                    Level::Info,
                    "[download] Destination: /home/Jos\u{fffd}/v.mkv".into()
                )),
                "{:?}",
                events.first()
            );
        ...
        /// The real yt-dlp is a PyInstaller bootloader with the actual
        /// program as its child (checked with `ps` on yt-dlp_linux), so
        /// killing only the direct child left the download running.
        #[test]
        fn cancelling_also_stops_what_yt_dlp_started() {
            let dir = crate::testing::tempdir();
            let pid_file = dir.join("grandchild.pid");
            let (dir2, paths) = fake_ytdlp(
                &format!(
                    "sh -c 'echo $$ > {0}; exec sleep 60' &\n\
            ...
            assert!(!alive, "the grandchild (pid {pid}) survived the cancel");
            fs::remove_dir_all(dir).unwrap();
            fs::remove_dir_all(dir2).unwrap();
        }

        /// Stop pressed before yt-dlp is even running must still stop it,
        /// not wait for it to finish on its own.
        #[test]
        fn a_cancel_that_arrives_before_the_start_is_honoured() {
            let (dir, paths) = fake_ytdlp("echo started\nsleep 8", 0);
            let cancel = Cancel::default();
            cancel.cancel();
            let started = std::time::Instant::now();
