...
pub struct Subtitles {
    ...
    /// Empty means every language except the live chat, which on a stream
    /// is the whole chat replay rather than subtitles.
    pub languages: String,
    pub write_file: bool,
    pub embed: bool,
    /// `--write-auto-subs`: the site's own generated subtitles too.
...
pub struct Options {
    ...
    /// `--cookies-from-browser`: `BROWSER[:PROFILE]`, for videos that need
    /// the user to be signed in (age restricted, members only...).
...
pub fn thumbnail_plan(o: &Options) -> ThumbnailPlan {
    ...
    // which for YouTube is often webm. It is rewrapped into the container
    // the user picked for merging, so choosing MP4 never quietly gives an
    // .mkv (VP9 in MP4 with a cover was checked on a real download).
    if o.format == "bestaudio" || o.format.starts_with("bestaudio/") {
        ThumbnailPlan::EmbedWithRewrap("webm>opus")
    } else if o.merge_output_format.as_deref() == Some("mp4") {
        ThumbnailPlan::EmbedWithRewrap("webm>mp4")
    } else {
...
pub fn build_args(
    ...
    let mut push = |s: &str| a.push(s.into());
    if let Some(browser) = &o.cookies_from_browser {
